...
def get_evaporation_ts():
    write_to = rf"data\evaporation"
    file_name = rf"evaporation_from_west.dat"
    df_west = pd.read_csv(
    ...
    # df_west.index = df_west.index.round("15MIN") # Rounding gives outof sequence error in SWMM (2 values have same time)
    evaporation_ts = df_west[".c_106.evaporation.Evaporation"].astype(float)
    file_path = os.path.join(write_to, file_name)
    with open(file_path, "w") as f:
        # Rain gauge definition
        f.write(f"; Evaporation data\n")

        for timestamp, value in evaporation_ts.items():
            if isnan(value):
                value = 0
            year = timestamp.year
            month = timestamp.month
            ...
            hour = timestamp.hour
            minute = timestamp.minute
            f.write(f"{month}/{day}/{year}\t{hour}:{minute}\t{value:.6f}\n")
